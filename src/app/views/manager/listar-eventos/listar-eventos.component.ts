...
export class ListarEventosComponent {
  ...
  ngOnInit(): void {
   this.UsuarioObtenido = this.apiLogin.getUser();
    ...
    this.apiService.obtenerGerenteByIdUsuario(this.UsuarioObtenido.idUsuario).subscribe(
      (data)=>{
        this.gerenteObtenido = data;
        this.apiService.listarEventosGerente(this.gerenteObtenido.id).subscribe(
          (data) => {
